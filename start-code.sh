...
img_output="deepexi/tt-content-center"         # output image tag
...
sudo mv $proj_home/tt-content-center-provider/target/tt-content-center-provider-*.jar $proj_home/tt-content-center-provider/target/demo.jar # 兼容所有sh脚本
...
# 删除容器
docker rm -f tt-content-center &> /dev/null
...
docker run -d --restart=on-failure:5 --privileged=true \
    ...
    -e SW_AGENT_NAME=tt-content-center \
    -e SW_AGENT_COLLECTOR_BACKEND_SERVICES=${skywalking_addr} \
    --name tt-content-center deepexi/tt-content-center \
    java \
        -Xmx${mem}m \
        -Xms${mem}m \
